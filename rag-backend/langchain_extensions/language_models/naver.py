...
class SkillsetHyperClova(BaseChatModel):
    """A chat model that uses the Hyper Clova language model."""
    ...
    def get_lc_namespace(cls) -> List[str]:
        """Get the namespace of the langchain object."""
        return ["langchain", "llms", "hyperclova"]
        ...
        **kwargs: Any,
        ...
        with httpx.stream(**request_params) as response:
            for line in response.iter_lines():
                ...
                if "status" in content and "message" in content["status"]:
                    logger.error(content["status"]["message"])

                if "finalAnswer" in content:
                    continue

                if selected_skills := content.get("selectedSkill"):
                    for skill in selected_skills:
                        plan_message = f"```Plan: {skill['nameForHuman']}```"
                        message_chunk = AIMessageChunk(content=plan_message)
                        chunk1 = ChatGenerationChunk(message=message_chunk)
                        chunks.append(chunk1)
                        newline_chunk = AIMessageChunk(content="\n\n\n")
                        chunk2 = ChatGenerationChunk(message=newline_chunk)
                        chunks.append(chunk2)

                if api_result := content.get("apiResult", []):
                    api_message = f"```API Result: {str(api_result)}```"
                    message_chunk = AIMessageChunk(content=api_message)
                    chunk1 = ChatGenerationChunk(message=message_chunk)
                    chunks.append(chunk1)
                    newline_chunk = AIMessageChunk(content="\n\n\n")
                    chunk2 = ChatGenerationChunk(message=newline_chunk)
        ...
        async with httpx.AsyncClient() as client:
            async with client.stream(**request_params) as response:
                async for line in response.aiter_lines():
                    ...
                    if "status" in content and "message" in content["status"]:
                        error_message = content["status"]["message"]
                        message_chunk = AIMessageChunk(content=error_message)
                        chunk = ChatGenerationChunk(message=message_chunk)
                        yield chunk
                    ...
                    if api_result := content.get("apiResult", []):
                        api_result = json.dumps(
                            api_result, indent=2, ensure_ascii=False
                        )
                        api_result = api_result.replace('\\"', "'")
                        api_message = f"```API Result: {api_result}\n```"
                        message_chunk = AIMessageChunk(content=api_message)
                        chunk1 = ChatGenerationChunk(message=message_chunk)
                        chunks.append(chunk1)
                        newline_chunk = AIMessageChunk(content="\n\n\n")
    ...
    def _create_params(
        ...
        params["query"] = last_message.content
        filtered_messages = [x for x in messages if not isinstance(x, SystemMessage)]
        params["chatHistory"] = [_convert_message_to_dict(m) for m in filtered_messages]
        params["tokenStream"] = str(stream)
        return params
    ...
    def _create_request_params(
        ...
        **kwargs: Any,
        ...
        return {
            ...
            "url": self.endpoint,
            "headers": headers,
            "json": self._create_params(messages, stop, stream=stream),
            "timeout": 120,
        }
